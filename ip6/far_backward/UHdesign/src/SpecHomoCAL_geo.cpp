...
tuple<Volume, Position> build_module(const Detector& lccdd, const xml::Component& mod_x, SensitiveDetector& sens);
...
static Ref_t create_detector(Detector& lccdd, xml_h e, SensitiveDetector sens){
	...
	auto [modVol, modSize] = build_module(lccdd, x_det.child( _Unicode(module) ) , sens);
...
//DECLARE_DETELEMENT(BotHomoCAL, create_detector)
